...
namespace seqslam {
    namespace opencl {
        namespace {
            auto toNdRange(std::vector<std::size_t> const& range) -> cl::NDRange {
                if (range.size() == 1) {
                    ...
                    throw std::runtime_error{fmt::format("Invalid number of dimensions ({}) "
                                                         "for NDRange. Must be between 1 and 3",
                                                         range.size())};
                }
            }
        ...
        Buffer::Buffer(Context const& context, std::size_t size, Access access)
            : size_{size}, buffer_{context.context(),
                                   toClAccess(access) | CL_MEM_ALLOC_HOST_PTR,
                                   size},
